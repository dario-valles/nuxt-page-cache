...
module.exports = function pageCache(_nuxt, _options) {
    ...
    function defaultCacheKeyBuilder(route, context) {
        ...
        const cacheKey =
            config.useHostPrefix === true && hostname
    ...
    function buildCacheKey(route, context) {
        ...
        const keyConfig = (config.key || defaultCacheKeyBuilder)(
            route,
            context
        );
        return {
    ...
    const cacheStatusHeader =
        typeof config.cacheStatusHeader === "string" &&
        config.cacheStatusHeader;
    const setHeaderFunc =
        typeof config.setHeaderFunc === "string"
            ? config.setHeaderFunc
            : "setHeader";
    const currentVersion =
        config.version || (this.options && this.options.version);
        ...
        if (purgeQueryParam && purgeSecret) {
            ...
            if (matches) {
                ...
                if (canPurge) {
                    ...
                    return cache.keysAsync(`${cacheKey}*`).then((keys) => {
                        if (keys && keys.length) {
                            return cache
                                .delAsync(keys)
