...
module.exports = function pageCache(_nuxt, _options) {
  ...
  renderer.renderRoute = function (route, context) {
    ...
    function renderSetCache() {
      return renderRoute(route, context).then(function (result) {
        ...
        if (!result.error && !result.redirected) {
          ...
          cache.setAsync(cacheKey, zlib.deflateSync(serialize(result)), {
            ttl: ttl
    ...
    var cacheRes = cache.getAsync(cacheKey).then(function (cachedResult) {
      if (cachedResult) {
        // decompress cachedResult with zlib
        setHeader(deserialize(zlib.inflateSync(cachedResult)), "HIT");
